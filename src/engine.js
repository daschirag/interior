...
(function () {
  ...
  (function preload() {
    ...
    function hide() { pl.classList.add("done"); setTimeout(function () { pl.style.display = "none"; }, 850); }
    setTimeout(function () { clearInterval(iv); hide(); }, 3200); // safety
  })();
  ...
  function apply(p) {
    ...
    if (portal) portal.style.opacity = String(clamp(1 - (p - 0.58) * 3.4, 0, 1));
    if (bloom) bloom.style.opacity = String(0.45 + ease(clamp(p * 1.2, 0, 1)) * 0.5);
    ...
    // hero text sits inside the doorway, then rushes past as we step through
    if (hero) {
      var ho = clamp(1 - p / 0.30, 0, 1);
      hero.style.opacity = String(ho);
      hero.style.transform = "translate(-50%,-50%) scale(" + (1 + p * 2.6) + ")";
    }
